...
vim.keymap.set('v', 'j', 'gj', { desc = '' })
vim.keymap.set('v', 'k', 'gk', { desc = '' })
...
-- select and print a random line from the visual selection
...
-- copy quickmark
vim.keymap.set('n', '<C-y>', 'g_F|lvg_"+y', { desc = '' })

-- :nnoremap <F, silent = true2> :let @+=system("awk -F'|' '{print $NF}' \| tr ' ' '\\n'", getline('.'))<CR>

-- search
vim.keymap.set("x", "/", '"vy/<C-r>v<CR>', { desc = "Search selected text" })
